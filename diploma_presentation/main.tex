...
\begin{frame}{Стохастическая модель RBC (DSGE)}
    \scriptsize
    \textbf{Отличие от детерминированной модели:}
    \begin{itemize}
        \item В производственную функцию добавлен случайный процесс $z_t$ (СФП):
        \[
        Y_t = A K_t^{\alpha} L_t^{1-\alpha} \cdot e^{z_t}
        \]
    \end{itemize}
    \textbf{Процесс для совокупной факторной производительности (AR(1)):}
    \[
    z_t = \rho z_{t-1} + \varepsilon_t,\qquad \varepsilon_t \sim \mathcal{N}(0,\sigma^2),\quad 0 \le \rho < 1
    \]
    \textbf{Остальные уравнения} --- как в детерминированной модели (условия первого порядка, бюджетное ограничение, закон движения капитала).\\
    \textbf{Методы решения:}
    \begin{itemize}
        \item \textbf{Локальное решение (Dynare):} метод возмущений --- линеаризация вокруг стационарного состояния.
        \item \textbf{Глобальное решение:} аппроксимация функции потребления $C_t = f(K_t, z_t)$ полиномами Чебышёва.
    \end{itemize}
    \textbf{Результат:} в стохастической модели --- ансамбль траекторий (распределение), а не единственная кривая.
\end{frame}

\begin{frame}{Решение стохастической DSGE-модели}
\end{frame}
