...
def plot_time_series(path, ss, prefix='case'):
    t = np.arange(len(path['K']))
    mapping = [('K', 'K_t'), ('C', 'C_t'), ('L', 'L_t'), ('Y', 'Y_t')]
    for key, label in mapping:
        fig, ax = plt.subplots(figsize=(8, 5))
        ax.plot(t, path[key], linewidth=2, label=label)
        ax.axhline(ss[f'{key}_star'], linestyle='--', linewidth=1.5, label=f'{label}^*')
        ax.set_xlabel('t')
        ax.set_ylabel(label)
        ax.set_title(f'Динамика {label}')
        ax.grid(True, alpha=0.3)
        ax.legend()
        fig.tight_layout()
        fig.savefig(f'{prefix}_{key}.png', dpi=180)
        plt.close(fig)


def plot_3d_plotly(path, ss, filename_html='trajectory_3d.html'):
    fig = go.Figure()
    ...
    fig.add_trace(go.Scatter3d(
        ...
        hovertemplate='K=%{x:.4f}<br>C=%{y:.4f}<br>L=%{z:.4f}<extra></extra>'
    ))
    ...
    fig.update_layout(
        title='3D-траектория модели RBC: (K_t, C_t, L_t)',
        scene=dict(
            xaxis_title='K_t',
            yaxis_title='C_t',
            zaxis_title='L_t',
...
if __name__ == '__main__':
    ...
    print(f'\nK0 = {K0:.10f}')
    print(f'C0 = {C0:.10f}')
    print(f'objective = {obj:.8e}')

    plot_time_series(path, ss, prefix='rbc_case')
    plot_3d_plotly(path, ss, filename_html='rbc_trajectory_3d.html')
    print('\nФайлы сохранены: rbc_case_K.png, rbc_case_C.png, rbc_case_L.png, rbc_case_Y.png, rbc_trajectory_3d.html')
